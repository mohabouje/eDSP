...
#include "edsp/thirdparty/CLI/ConfigFwd.hpp"
#include "edsp/thirdparty/CLI/Error.hpp"
#include "edsp/thirdparty/CLI/FormatterFwd.hpp"
#include "edsp/thirdparty/CLI/Macros.hpp"
#include "edsp/thirdparty/CLI/Option.hpp"
#include "edsp/thirdparty/CLI/Split.hpp"
#include "edsp/thirdparty/CLI/StringTools.hpp"
#include "edsp/thirdparty/CLI/TypeTools.hpp"
 ...
 * program, so that the options can be evaluated and the help option doesn't accidentally run your program. */
    ...
    public:
        ...
        ///
        Option* add_option(std::string name, callback_t callback, std::string description = "",
        ...
        template <typename T>
        Option* add_complex(std::string name, T& variable, std::string description = "", bool defaulted = false,
        ...
        Option* set_config(std::string name = "", std::string default_filename = "",
                           std::string help = "Read an ini file", bool required = false) {
        ...
        App* set_footer(std::string msg) {
            return footer(msg);
        }

        /// Provided for backwards compatibility \deprecated
        CLI11_DEPRECATED("Please use name instead")
        App* set_name(std::string msg) {
            return name(msg);
        }

        /// Provided for backwards compatibility \deprecated
        CLI11_DEPRECATED("Please use callback instead")
        App* set_callback(std::function<void()> fn) {
            return callback(fn);
        }
        ...
        size_t remaining_size(bool recurse = false) const {
            auto count = static_cast<size_t>(std::count_if(std::begin(missing_), std::end(missing_),
    ...
    protected:
        ...
        bool _parse_single_config(const ConfigItem& item, size_t level = 0) {
            if (level < item.parents.size()) {
                ...
                } catch (const OptionNotFound&) { return false; }
        ...
        void _parse_arg(std::vector<std::string>& args, bool second_dash) {
            ...
            auto op_ptr =
                std::find_if(std::begin(options_), std::end(options_), [name, second_dash](const Option_p& opt) {
