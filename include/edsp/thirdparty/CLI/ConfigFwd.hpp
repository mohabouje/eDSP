    ...
    namespace detail {
        ...
        inline std::string ini_join(std::vector<std::string> args) {
            ...
            for (const auto& arg : args) {
                ...
                auto it =
                    std::find_if(arg.begin(), arg.end(), [](char ch) { return std::isspace<char>(ch, std::locale()); });
                if (it == arg.end())
                    s << arg;
        ...
        virtual std::vector<std::string> to_flag(const ConfigItem& item) const {
            if (item.inputs.size() == 1) {
                ...
                if (val == "true" || val == "on" || val == "yes") {
                    ...
                    } catch (const std::invalid_argument&) { throw ConversionError::TrueFalse(item.fullname()); }
                }
            } else {
