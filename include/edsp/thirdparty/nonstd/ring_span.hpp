...
#    define ring_span_lite_VERSION                                         \
        ring_span_STRINGIFY(ring_span_lite_MAJOR) "." ring_span_STRINGIFY( \
            ring_span_lite_MINOR) "." ring_span_STRINGIFY(ring_span_lite_PATCH)
...
#    endif
        ...
        ring_span(ContiguousIterator begin, ContiguousIterator end, ContiguousIterator first, size_type size,
                  Popper popper = Popper()) nsrs_noexcept : m_data(&*begin),
                                                            m_size(size),
                                                            m_capacity(static_cast<size_type>(end - begin)),
...
#    if nsrs_CPP11_OR_GREATER
        ...
        template <class... Args>
        void emplace_back(Args&&... args) nsrs_noexcept_op((std::is_nothrow_constructible<T, Args...>::value &&
...
#        if nsrs_CPP11_OR_GREATER
        ...
        void emplace_front(Args&&... args) nsrs_noexcept_op((std::is_nothrow_constructible<T, Args...>::value &&
                                                             std::is_nothrow_move_assignable<T>::value)) {
            if (full())
                decrement_front_and_back_();
            else
