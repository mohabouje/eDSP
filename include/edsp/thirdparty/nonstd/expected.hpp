...
#    endif // nsel_P0323R
        ...
        public:
            ...
            constexpr explicit unexpected_type(unexpected_type<E2>&& error nsel_REQUIRES_A(
                std::is_constructible<E, E2&&>::value && !std::is_convertible<E2&&, E>::value /*=> explicit */)) :
                m_error(error) {}

            template <typename E2>
            constexpr /*non-explicit*/ unexpected_type(unexpected_type<E2>&& error nsel_REQUIRES_A(
                std::is_constructible<E, E2&&>::value&& std::is_convertible<E2&&, E>::value /*=> non-explicit */)) :
                m_error(error) {}
...
#    else
                std::is_nothrow_move_constructible<E>::value&& noexcept(std::swap(std::declval<E&>(),
                                                                                  std::declval<E&>()))
#    endif
            ) {
                using std::swap;
...
#    if nsel_P0323R <= 2
        ...
        public:
            ...
            explicit unexpected_type(std::exception_ptr const& error) : m_error(error) {}

            explicit unexpected_type(std::exception_ptr&& error) : m_error(std::move(error)) {}

            template <typename E>
            explicit unexpected_type(E error) : m_error(std::make_exception_ptr(error)) {}
...
#    if nsel_P0323R <= 3
        ...
        /*nsel_constexpr14*/ auto inline make_unexpected_from_current_exception()
...
#    endif // nsel_P0323R
        ...
        public:
            ...
            nsel_REQUIRES_0(std::is_default_constructible<T>::value) nsel_constexpr14
            ...
            nsel_constexpr14 explicit expected(expected<U, G> const& other nsel_REQUIRES_A(
                std::is_constructible<T, const U&>::value&& std::is_constructible<E, const G&>::value &&
            ...
            template <typename U = T>
            nsel_constexpr14 expected(U&& value nsel_REQUIRES_A(
                std::is_constructible<T, U&&>::value &&
                !std::is_same<typename std20::remove_cvref<U>::type, nonstd_lite_in_place_t(U)>::value &&
                !std::is_same<expected<T, E>, typename std20::remove_cvref<U>::type>::value &&
                !std::is_same<nonstd::unexpected_type<E>, typename std20::remove_cvref<U>::type>::value &&
            ...
            nsel_constexpr14 explicit expected(nonstd_lite_in_place_t(T), Args&&... args) : has_value_(true) {
            ...
            nsel_constexpr14 explicit expected(unexpect_t, Args&&... args) : has_value_(false) {
...
#    if nsel_CPP17_OR_GREATER
                std::is_nothrow_move_constructible<T>::value&& std::is_nothrow_swappable<T&>::value&&
                    std::is_nothrow_move_constructible<E>::value&& std::is_nothrow_swappable<E&>::value
#    else
                std::is_nothrow_move_constructible<T>::value&& noexcept(std::swap(std::declval<T&>(),
                                                                                  std::declval<T&>())) &&
                std::is_nothrow_move_constructible<E>::value &&
...
#    endif
            ...
            template <
                typename U nsel_REQUIRES_T(std::is_copy_constructible<T>::value&& std::is_convertible<U&&, T>::value)>
            value_type value_or(U&& v) const& {
                return has_value() ? contained.value() : static_cast<T>(std::forward<U>(v));
            }

            template <
                typename U nsel_REQUIRES_T(std::is_move_constructible<T>::value&& std::is_convertible<U&&, T>::value)>
        ...
        public:
            ...
            nsel_constexpr14 expected(expected const& other) : has_value_(other.has_value_) {
                if (!has_value())
                    contained.construct_error(other.contained.error());
            }

            nsel_REQUIRES_0(std::is_move_constructible<E>::value) nsel_constexpr14
                expected(expected&& other) noexcept(true // TBD - see also non-void specialization
                                                    ) :
                has_value_(other.has_value_) {
                if (!has_value())
                    contained.construct_error(std::move(other.contained.error()));
            ...
            constexpr explicit expected(nonstd_lite_in_place_t(void)) : has_value_(true) {}
            ...
            template <typename G = E>
            nsel_constexpr14 explicit expected(
                nonstd::unexpected_type<G>&& error nsel_REQUIRES_A(!std::is_convertible<G&&, E>::value /*=> explicit */
                                                                   )) :
                has_value_(false) {
                contained.construct_error(std::move(error.value()));
            }
...
#    if nsel_P0323R <= 3
        ...
        auto make_expected_from_call(F f,
            ...
            } catch (...) { return make_unexpected_from_current_exception(); }
        }
