...
#if defined(span_CONFIG_SELECT_STD_SPAN) && span_CONFIG_SELECT_STD_SPAN && defined(span_CONFIG_SELECT_NONSTD_SPAN) && \
    span_CONFIG_SELECT_NONSTD_SPAN
...
#        define span_COMPILER_CLANG_VERSION \
            span_COMPILER_VERSION(__clang_major__, __clang_minor__, __clang_patchlevel__)
#    else
...
#    define span_CONTRACT_CHECK(type, cond)                                                                     \
        cond ? static_cast<void>(0)                                                                             \
             : nonstd::span_lite::detail::report_contract_violation(span_LOCATION(__FILE__, __LINE__) ": " type \
                                                                                                      " violation.")
...
#    if span_HAVE(CONSTRAINED_SPAN_CONTAINER_CTOR)
    ...
    template <
        ...
        class = typename std::enable_if<
            !detail::is_span<Container>::value && !detail::is_array<Container>::value &&
...
#    if span_HAVE(DEFAULT_FUNCTION_TEMPLATE_ARG)
        template <bool Dependent = false, class = typename std::enable_if<(Dependent || Extent <= 0)>::type>
#    endif
        ...
        span_constexpr_exp span(pointer ptr, index_type count) : data_(ptr), size_(count) {
            span_EXPECTS((ptr == span_nullptr && count == 0) || (ptr != span_nullptr && count >= 0));
        }

        span_constexpr_exp span(pointer firstElem, pointer lastElem) :
            data_(firstElem),
            size_(std::distance(firstElem, lastElem)) {
            span_EXPECTS(std::distance(firstElem, lastElem) >= 0);
        }
...
#    if span_HAVE(DEFAULT_FUNCTION_TEMPLATE_ARG)
            ,
            class = typename std::enable_if<(Extent == dynamic_extent || Extent == N) &&
                                            std::is_convertible<value_type (*)[], element_type (*)[]>::value>::type
#    endif
            >
...
#        endif
            >
        span_constexpr span(std::array<value_type, N> const& arr) span_noexcept : data_(span_ADDRESSOF(arr[0])),
                                                                                  size_(to_size(arr.size())) {
        }
...
#    if span_HAVE(CONSTRAINED_SPAN_CONTAINER_CTOR)
        template <class Container,
                  class = typename std::enable_if<can_construct_from<Container, element_type>::value>::type>
        span_constexpr span(Container& cont) : data_(cont.data()), size_(to_size(cont.size())) {}

        template <class Container,
                  class = typename std::enable_if<std::is_const<element_type>::value &&
                                                  can_construct_from<Container, element_type>::value>::type>
        span_constexpr span(Container const& cont) : data_(cont.data()), size_(to_size(cont.size())) {}
...
#    if span_HAVE(DEFAULT_FUNCTION_TEMPLATE_ARG)
                  ...
                  class = typename std::enable_if<
                      ...
                      std::is_convertible<OtherElementType (*)[], element_type (*)[]>::value>::type
#    endif
                  >
...
#    endif
        span_constexpr_exp span<element_type, Count> subspan() const {
            span_EXPECTS((0 <= Offset && Offset <= size()) &&
        ...
        span_constexpr_exp span<element_type, dynamic_extent> subspan(index_type offset,
...
#    if span_HAVE(BYTE)
    ...
    inline span_constexpr
        span<const std::byte, ((Extent == dynamic_extent) ? dynamic_extent : (to_size(sizeof(T)) * Extent))>
...
namespace nonstd { namespace span_lite {
    ...
    template <class T>
    inline span_constexpr span<T> make_span(T* ptr, index_t count) span_noexcept {
    ...
    template <class T, size_t N>
    inline span_constexpr span<T, N> make_span(T (&arr)[N]) span_noexcept {
...
#    if span_USES_STD_SPAN || span_HAVE(ARRAY)
    ...
    template <class T, size_t N>
    inline span_constexpr span<T, N> make_span(std::array<T, N>& arr) span_noexcept {
        return span<T, N>(arr);
    }

    template <class T, size_t N>
    inline span_constexpr span<const T, N> make_span(std::array<T, N> const& arr) span_noexcept {
...
#    if span_USES_STD_SPAN || (span_HAVE(CONSTRAINED_SPAN_CONTAINER_CTOR) && span_HAVE(AUTO))
    ...
    inline span_constexpr auto make_span(Container& cont) span_noexcept -> span<typename Container::value_type> {
...
#    if !span_USES_STD_SPAN && span_FEATURE_TO_STD(WITH_CONTAINER)
    ...
    template <class Container>
    inline span_constexpr span<typename Container::value_type> make_span(with_container_t,
                                                                         Container& cont) span_noexcept {
        return span<typename Container::value_type>(with_container, cont);
    }

    template <class Container>
    inline span_constexpr span<const typename Container::value_type> make_span(with_container_t,
                                                                               Container const& cont) span_noexcept {
...
namespace nonstd { namespace span_lite {
    ...
    template <class T>
    inline span_constexpr auto byte_span(T& t) span_noexcept -> span<std::byte, sizeof(t)> {
        return span<std::byte, sizeof(t)>(reinterpret_cast<std::byte*>(&t), sizeof(t));
    }

    template <class T>
    inline span_constexpr auto byte_span(T const& t) span_noexcept -> span<const std::byte, sizeof(t)> {
