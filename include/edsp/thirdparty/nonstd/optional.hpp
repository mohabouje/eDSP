...
#        if optional_CPP11_OR_GREATER || optional_COMPILER_MSVC_VERSION >= 140 || \
            (optional_COMPILER_MSVC_VERSION >= 90 && _HAS_CPP0X)
...
#            define optional_REQUIRES_T(...) , typename = typename std::enable_if<__VA_ARGS__>::type

#            define optional_REQUIRES_R(R, ...) typename std::enable_if<__VA_ARGS__, R>::type

#            define optional_REQUIRES_A(...) , typename std::enable_if<__VA_ARGS__, void*>::type = optional_nullptr
...
#        if optional_CPP11_OR_GREATER
            ...
            optional_constexpr14 optional(optional&& other optional_REQUIRES_A(
...
#        if optional_CPP11_OR_GREATER
                noexcept(std::is_nothrow_move_constructible<T>::value&& noexcept(std::swap(std::declval<T&>(),
                                                                                           std::declval<T&>())))
#        endif
            {
                using std::swap;
