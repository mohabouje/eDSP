...
namespace edsp { namespace auditory {
    ...
    /**
     * @brief This function computes an array of N frequencies uniformly spaced between [x1,x2] on a Cent
    ...
    constexpr void centspace(OutputIt first, OutputIt last, Numeric min, Numeric max) {
        const auto N = std::distance(first, last);
        algorithm::linspace(first, min, max, N);
        std::transform(first, last, first, [](const auto data) { return converter::hertz2cent(data); });
    }
