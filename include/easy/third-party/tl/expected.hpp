...
#    define TL_TRAITS_MUTEX
        ...
        constexpr auto invoke(Fn&& f,
                              Args&&... args) noexcept(noexcept(std::forward<Fn>(f)(std::forward<Args>(args)...)))
        ...
        struct invoke_result_impl<F, decltype(detail::invoke(std::declval<F>(), std::declval<Us>()...), void()),
                                  Us...> {
...
// move constructible
        ...
        template <class T, class E,
                  bool = is_void_or<T, conjunction<TL_EXPECTED_IS_TRIVIALLY_COPY_ASSIGNABLE(T),
                      ...
                      TL_EXPECTED_IS_TRIVIALLY_COPY_ASSIGNABLE(E)::value&& TL_EXPECTED_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(
                          E)::value&& TL_EXPECTED_IS_TRIVIALLY_DESTRUCTIBLE(E)::value>
...
#    endif
        ...
        /// \synopsis template <class F>\nconstexpr auto and_then(F &&f) &&;
        template <class F>
        TL_EXPECTED_11_CONSTEXPR auto and_then(F&& f) && -> decltype(and_then_impl(std::move(*this),
                                                                                   std::forward<F>(f))) {
            return and_then_impl(std::move(*this), std::forward<F>(f));
        }
...
#    endif
    ...
    namespace detail {
        ...
        constexpr auto and_then_impl(Exp&& exp, F&& f) {
            ...
            return exp.has_value() ? detail::invoke(std::forward<F>(f)) : Ret(unexpect, exp.error());
        }
#else
        template <class>
        struct TC;
    ...
    template <
        class T, class E,
        detail::enable_if_t<std::is_move_constructible<T>::value && std::is_move_constructible<E>::value>* = nullptr>
