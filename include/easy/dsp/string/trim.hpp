 ...
 * Date: 05/10/2018
 */
#ifndef EASYDSP_TRIM_HPP
#define EASYDSP_TRIM_HPP
...
namespace easy { namespace dsp  { namespace string {
    ...
    */
    ...
    inline void ltrim(std::basic_string<Char>& str) {
        str.erase(std::begin(str), std::find_if(std::begin(str), std::end(str), [](Char character) {
    ...
    */
    ...
    inline void rtrim(std::basic_string<Char>& str) {
        str.erase(std::find_if(std::rbegin(str), std::rend(str), [](Char character) {
            return !std::isspace(character);
        }).base(), std::end(str));
    }
