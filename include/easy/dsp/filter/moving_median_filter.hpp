...
namespace easy { namespace dsp { namespace filter {
    ...
    public:
        ...
        inline explicit MovingMedianFilter(size_type window_size);
    ...
    template <typename T>
    inline typename MovingMedianFilter<T>::size_type MovingMedianFilter<T>::window_size() const {
        return buffer_.capacity();
    }

    template <typename T>
    inline void MovingMedianFilter<T>::set_window_size(size_type window_size) {
    ...
    template <typename T>
    template <typename BiIterator>
    inline void MovingMedianFilter<T>::filter(BiIterator first, BiIterator last) {
        filter(first, last, first);
    }

    template <typename T>
    template <typename InputIterator, typename OutputIterator>
    inline void MovingMedianFilter<T>::filter(InputIterator first, InputIterator last, OutputIterator out) {
