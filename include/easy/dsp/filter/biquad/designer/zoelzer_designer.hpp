...
namespace easy { namespace dsp { namespace filter {
    ...
    template <typename T>
    struct ZoelzerFilterDesigner<T, BiquadType::LowPass> {
    ...
    template <typename T>
    struct ZoelzerFilterDesigner<T, BiquadType::HighPass> {
        constexpr Biquad<T> operator()(T fc, T sample_rate, T Q, T gain_db = 1) const {
            meta::unused(gain_db);
            const auto K    = std::tan(constants<T>::pi * fc / sample_rate);
            const auto norm = 1 / (1 + K / Q + K * K);
    ...
    template <typename T>
    struct ZoelzerFilterDesigner<T, BiquadType::BandPassSkirtGain> {
        constexpr Biquad<T> operator()(T fc, T sample_rate, T Q, T gain_db = 1) const {
            meta::unused(gain_db);
            const auto K    = std::tan(constants<T>::pi * fc / sample_rate);
            const auto norm = 1 / (1 + K / Q + K * K);
    ...
    template <typename T>
    struct ZoelzerFilterDesigner<T, BiquadType::Notch> {
        constexpr Biquad<T> operator()(T fc, T sample_rate, T Q, T gain_db = 1) const {
            meta::unused(gain_db);
            const auto K    = std::tan(constants<T>::pi * fc / sample_rate);
            const auto norm = 1 / (1 + K / Q + K * K);
    ...
    template <typename T>
    struct ZoelzerFilterDesigner<T, BiquadType::PeakingEQ> {
        constexpr Biquad<T> operator()(T fc, T sample_rate, T Q, T gain_db = 1) const {
    ...
    template <typename T>
    struct ZoelzerFilterDesigner<T, BiquadType::LowShelf> {
        constexpr Biquad<T> operator()(T fc, T sample_rate, T Q, T gain_db = 1) const {
            meta::unused(Q);
            const auto V = db2mag(gain_db);
            const auto K = std::tan(constants<T>::pi * fc / sample_rate);
    ...
    template <typename T>
    struct ZoelzerFilterDesigner<T, BiquadType::HighShelf> {
        constexpr Biquad<T> operator()(T fc, T sample_rate, T Q, T gain_db = 1) const {
            meta::unused(Q);
            const auto V = db2mag(gain_db);
            const auto K = std::tan(constants<T>::pi * fc / sample_rate);
