...
namespace easy { namespace dsp { namespace random {
    ...
    struct PerlinNoiseGenerator {
        ...
        inline result_type operator()() {
            ...
            return lerp(fade_value,
                        grad(permutation[index], floor_value),
                        grad(permutation[index + 1], floor_value - 1)) * 2;
        }
    private:
        Engine generator_;
        std::uniform_int_distribution<T> distribution_;
        static constexpr const std::array<T, 512> permutation = {{ 151,160,137,91,90,15,
