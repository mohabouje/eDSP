...
template <typename T>
IDCT<T>::IDCT(size_t size, DCT_Type t) : AlgorithmTemplate(true, Error()), t(t) {
    setSize(size);
}
...
template <typename T>
void IDCT<T>::setSize(size_t size) {
    input.resize(size);
    output.resize(size);
    setType(t);
...
template<typename T>
const std::vector<T> &IDCT<T>::compute(const std::vector<T> &data) {
...
EDSP_DCL_TEMPLATE(eDSP::transforms::IDCT, double);