    ...
    namespace Math {
        namespace Container {
            ...
            hasNegative(const Container& array) {
                return std::find_if(array.begin(), array.end(),  Util::isNegative<typename Container::value_type>) != array.end();
            }

            TEMPLATE_CONTAINER(Container, bool)
            hasZero(const Container& array) {
                return std::find_if(array.begin(), array.end(), Util::isZero<typename Container::value_type>) != array.end();
            }
            ...
            ones(const std::size_t& size) {
                Container tmp(size);
                set<double>(tmp, static_cast<typename Container::value_type>(1));
                return tmp;
            }
