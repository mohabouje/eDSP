...
void pffft_validate_N(int N, int cplx) {
    ...
    if (!s) {
        printf("Skipping N=%d, not supported\n", N);
        return;
    }
    ...
    for (pass = 0; pass < 2; ++pass) {
...

        {
            ...
            if (pass == 0)
                pffft_transform(s, tmp, out, 0, PFFFT_BACKWARD);
            else
                pffft_transform_ordered(s, tmp, out, 0, PFFFT_BACKWARD);
            memcpy(tmp2, out, Nbytes);
            memcpy(out, tmp, Nbytes);
            if (pass == 0)
                pffft_transform(s, out, out, 0, PFFFT_BACKWARD);
            else
...
void pffft_validate(int cplx) {
    static int Ntest[] = {16,  32,      64,  96,  128,  160,  192,  256,  288,  384,   5 * 96, 512,
                          576, 5 * 128, 800, 864, 1024, 2048, 2592, 4000, 4096, 12000, 36864,  0};
    int k;
    for (k = 0; Ntest[k]; ++k) {
        int N = Ntest[k];
        if (N == 16 && !cplx)
...
void benchmark_ffts(int N, int cplx) {
    ...
    {
        float* wrk    = malloc(2 * Nbytes + 15 * sizeof(float));
        int max_iter_ = max_iter / pffft_simd_size();
        if (max_iter_ == 0)
            max_iter_ = 1;
        if (cplx)
            cffti(N, wrk);
        ...
        flops = (max_iter_ * 2) *
                ((cplx ? 5 : 2.5) * N * log((double) N) / M_LN2); // see http://www.fftw.org/speed/method.html
...
int main(int argc, char** argv) {
    int Nvalues[] = {64,   96,   128,  160,  192,  256,      384,   5 * 96, 512,        5 * 128,     3 * 256, 800,
                     1024, 2048, 2400, 4096, 8192, 9 * 1024, 16384, 32768,  256 * 1024, 1024 * 1024, -1};
    int i;
